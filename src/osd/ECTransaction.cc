...
void encode_and_write(
  ...
  for (auto &&i : *transactions) {
   ...
   if(overwrite && (write_sid.find(i.first) == write_sid.end())) {
     ldpp_out(dpp,20) << __func__ << ": transactions is continue write_sid=" << write_sid << "  i.first=" << i.first << dendl;
     continue;
   }
  ...
  bool &have_append)
{
  ...
  t.safe_create_traverse(
	...
	ldpp_dout(dpp, 20) << __func__ << ": to_write="<<to_write<<" off="<<off<<" len="<<len<<" b1="<<b1<< dendl;
	to_write.insert(off, len, bl);
	ldpp_dout(dpp, 20) << __func__ << ": after insert to_write="<<to_write<<" b1="<<b1<< dendl;
	if (end > new_size)
	  new_size = end;
      }
